...
覺得時間過太快、食物掉太兇、怪物一下就圍上來?按 `F6` 開設定視窗即時調整:

![F6 設定視窗](docs/screenshots/f6_settings.png)

- **遊戲速度（時間流速）**:預設 **40%** ── 控制食物消耗、怪物移動、時間之門的快慢。越低越慢、越從容。
- **怪物產生率**:預設 **55%** ── 控制地面遇怪的頻率。
- 操作:`↑/↓` 選列、`←/→` 增減 5%（範圍 10–200%）、`F6` 或 `ESC` 關閉。設定會存進存檔,下次自動沿用。
- 想要原版原始節奏?兩項都設成 100%。機制細節見 [`docs/GAME-MECHANICS.md`](docs/GAME-MECHANICS.md)。
