...
#ifdef HAVE_SDL_MIXER
/* [正典:enchant.exp RE(Ghidra)+ 影片觀察 + CD /SOUND/ 資產 2026-06-13]
   U2 FM Towns 音樂兩套來源:
   (1) CDDA(合集 CD track05=U2 intro / track07=U2 結局)── 只在 intro/結局播。
   (2) EUP(FM Towns EUPHONY 序列,CD /SOUND/*.EUP + ULTIMA.FMB)── 遊玩配樂,檔名即場景:
       MAP=地面 · TOWN=城鎮 · DUNGEON(DANJON)=地牢 · OSIRO=城堡。
   EUP 需離線算繪成 OGG(YM2612+RF5C68 + FMB)放進 --music 目錄,檔名見下表;未算繪則該場景靜音。
   ctx: 0 intro(track05,播一次)/ 1 結局(track07,循環)/ 2 地面 / 3 城鎮 / 4 地牢 / 5 城堡(皆循環)。 */
static const char *MUS_TRACK[6] = {
    "track05",      /* 0 intro(CDDA)*/
    "track07",      /* 1 ending(CDDA)*/
    "eup_map",      /* 2 overworld(EUP MAP)*/
    "eup_town",     /* 3 town(EUP TOWN)*/
    "eup_dungeon",  /* 4 dungeon(EUP DANJON)*/
    "eup_osiro",    /* 5 castle(EUP OSIRO)*/
};
static void mus_play(int audio_ok, const char *dir, int ctx, Mix_Music **bgm, int *cur)
{
    if (!audio_ok || ctx==*cur || ctx<0 || ctx>5) return;
    *cur = ctx;
    if (*bgm){ Mix_HaltMusic(); Mix_FreeMusic(*bgm); *bgm=NULL; }
    if (!dir) return;
    char p[600]; snprintf(p,sizeof p,"%s/%s.ogg",dir,MUS_TRACK[ctx]);
    *bgm = Mix_LoadMUS(p);                          /* 缺檔(EUP 未算繪)→ NULL → 靜音 */
    if (*bgm) Mix_PlayMusic(*bgm, ctx==0?0:-1);     /* intro 播一次,其餘循環 */
}
/* 城堡偵測:Lord British 城堡 mapx80(LOC_REG 'c' 類→ mapx80/82/83…);此處以城鎮編號近似。 */
static int mus_ctx(const Game *g)
{
    if (g->won) return 1;                                  /* 結局 → track07 */
    if (g->mode==MODE_DUNGEON) return 4;                   /* 地牢 → DUNGEON */
    if (g->mode==MODE_SPACE)  return 2;                    /* 太空:暫用地面曲(無專屬 EUP)*/
    if (g->in_town){
        /* 城堡(Lord British / 國王)用 OSIRO,一般城鎮用 TOWN */
        if (!strcmp(g->town_loaded,"80")||!strcmp(g->town_loaded,"82")||
            !strcmp(g->town_loaded,"83")) return 5;        /* 城堡 → OSIRO */
        return 3;                                          /* 城鎮 → TOWN */
    }
    return 2;                                              /* 地面 → MAP */
}
